...
export default async function HomePage() {
  ...
  const [
    ...
    supabase.from("stages").select("id, stage_number, stage_date, stage_type, departure, arrival, distance_km").neq("status", "locked").order("stage_number", { ascending: true }).limit(1).single(),
  ]);
  ...
  const nextStage = nextStageRes.data ?? null;
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-3">
        ...
        {/* Volgende etappe — vooruitblik */}
        {nextStage ? (
          <div className="rounded-2xl border border-[#E2DFF0] bg-white p-5 shadow-sm flex flex-col">
            <h2 className="mb-3 text-xs font-semibold uppercase tracking-wide text-[#6B7280]">Volgende etappe</h2>
            <div className="flex items-center gap-2 mb-2">
              <span className="rounded bg-[#9462A6] px-2 py-0.5 text-xs font-bold text-white">E{nextStage.stage_number}</span>
              <span className="rounded bg-[#F3F4F6] px-1.5 py-0.5 text-xs text-[#6B7280]">
                {TYPE_LABEL[nextStage.stage_type] ?? nextStage.stage_type}
              </span>
            </div>
            <p className="text-sm font-semibold text-[#111827] mb-1">
              {nextStage.departure && nextStage.arrival
                ? `${nextStage.departure} → ${nextStage.arrival}`
                : "Route nog onbekend"}
            </p>
            <p className="text-xs text-[#6B7280] mb-1">
              {new Date(nextStage.stage_date).toLocaleDateString("nl-NL", { weekday: "long", day: "numeric", month: "long" })}
            </p>
            {(nextStage as { distance_km?: number | null }).distance_km && (
              <p className="text-xs text-[#9CA3AF] mb-3">
                {(nextStage as { distance_km?: number | null }).distance_km} km
              </p>
            )}
            <div className="mt-auto pt-3 border-t border-[#F3F4F6]">
              <a
                href={`https://www.wielerflits.nl/?s=tour+de+france+2026+etappe+${nextStage.stage_number}+voorbeschouwing`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 rounded-xl bg-[#EDE8F5] px-3 py-2 text-xs font-semibold text-[#5760A6] hover:bg-[#D8D3EC] transition w-full justify-center"
              >
                <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                </svg>
                Voorbeschouwing op WielerFlits
              </a>
            </div>
          </div>
        ) : (
          <div className="rounded-2xl border border-[#E2DFF0] bg-white p-5 shadow-sm">
            <h2 className="mb-3 text-xs font-semibold uppercase tracking-wide text-[#6B7280]">Volgende etappe</h2>
            <p className="text-sm text-[#9CA3AF]">Alle etappes zijn gereden</p>
          </div>
        )}
      </div>
    </div>
