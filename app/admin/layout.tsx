...
const adminLinks = [
  ...
  { href: "/admin/verslagen", label: "Verslagen" },
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="mb-6 flex items-center gap-2">
        <span className="rounded-md bg-[#EDE8F5] px-2 py-0.5 text-xs font-semibold text-[#5760A6]">
          Admin
        </span>
          ...
          <div className="flex gap-1 overflow-x-auto lg:flex-col">
            {adminLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium text-[#374151] hover:bg-[#EDE8F5] transition"
              >
                {link.label}
              </Link>
            ))}
