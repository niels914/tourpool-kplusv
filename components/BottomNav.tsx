...
export function BottomNav({ lastLockedStageId, isAdmin = false }: Props) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-[#E2DFF0] bg-white/95 backdrop-blur-sm md:hidden">
      <div className="mx-auto flex max-w-lg items-end justify-around px-2 py-2">
        ...
        {/* Chat (of Admin voor admins) */}
        {isAdmin ? (
          <Link href="/admin" className={`flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl transition ${isAdmin_ ? "text-[#9462A6]" : "text-[#9CA3AF] hover:text-[#5760A6]"}`}>
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
